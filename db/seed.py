...
def seed(force: bool = False) -> None:
    with Session(engine) as session:
        if force:
            ...
            print(
                "Markers already exist — nothing to do. Use --force to reload."
            )
            return

        for fixture in sample_markers:
            address = Address(
                **fixture.address.model_dump(exclude={"id", "created_at"})
            )
            session.add(
                Marker(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Load fixture markers into the database."
    )
    parser.add_argument(
        "--force", action="store_true", help="delete existing rows first"
    )
